...
const PROFILE = process.env.CHROME_PROFILE_DIR ?? `${process.env.TMPDIR ?? '/tmp'}/raid-shot-profile`;
...
const built = await page.evaluate(() => {
  ...
  const p = sim.player;
  const roster = [
    ['Brightoak', 'druid', 1], ['Stormcaller', 'shaman', 1], ['Nightblade', 'rogue', 1],
    ['Emberlyn', 'mage', 1], ['Holyverse', 'priest', 2], ['Ironhowl', 'warrior', 2],
    ['Grimfang', 'warlock', 2], ['Swiftarrow', 'hunter', 2],
  ];
  ...
  pids.forEach((p) => sim.partyByPid.set(p, party.id));
...
const raidRows = await page.evaluate(() =>
  document.querySelectorAll('#social-window .soc-body .raid-row, #social-window .soc-body [data-raid-pid]').length);
console.log('raid roster rows rendered:', raidRows);
console.log('raid tab text:\n' + await page.evaluate(() =>
