...
// guaranteed `pristine_ridge_stalker_pelt` trophy and the rare `cragmaw_huntcord`
...
await page.waitForFunction(() => window.__game && window.__game.sim && window.__game.hud, { timeout: 20000 });
...
const info = await page.evaluate(() => {
  ...
  sim.addItem('pristine_ridge_stalker_pelt', 1);
...
async function shotTooltip(label, file) {
  ...
  if (idx < 0) { console.log(`tooltip ${label}: row not found`); return; }
  ...
  const clip = await page.evaluate(() => {
    ...
    const rs = [tt, bg].filter((e) => e && e.style.display !== 'none').map((e) => e.getBoundingClientRect());
...
await shotTooltip("Pristine Ridge Stalker Pelt", 'tmp/cragmaw_trophy_tooltip.png');
