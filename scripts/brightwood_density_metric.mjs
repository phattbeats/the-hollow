...
// The remaining Brightwood Glade species.
const GLADE = new Set([
  'glade_fox',
  'spotted_fawn',
  'meadow_crane',
...
const mobs = [...sim.entities.values()].filter((e) => e.kind === 'mob' && GLADE.has(e.templateId));
...
const xs = mobs.map((m) => m.pos.x),
  zs = mobs.map((m) => m.pos.z);
...
const elwynClear = Math.min(...mobs.map((m) => Math.hypot(m.pos.x - elwyn.x, m.pos.z - elwyn.z)));

console.log(`Brightwood Glade: ${mobs.length} mobs (seed 1337)`);
console.log(`  footprint        : ${span(xs).toFixed(0)} x  by  ${span(zs).toFixed(0)} z`);
console.log(
  `  nearest-neighbour: mean ${mean.toFixed(2)}y, median ${median.toFixed(2)}y, min ${nn[0].toFixed(2)}y`,
);
