...
const PROFILE = process.env.CHROME_PROFILE_DIR ?? `${process.env.TMPDIR ?? '/tmp'}/raid2party-profile`;
fs.rmSync(PROFILE, { recursive: true, force: true });
fs.mkdirSync(PROFILE, { recursive: true });
const browser = await puppeteer.launch({
  executablePath: EDGE, headless: 'new', protocolTimeout: 120000,
  userDataDir: PROFILE,
  args: ['--window-size=1366,820', '--use-angle=swiftshader', '--enable-unsafe-swiftshader',
...
await page.waitForFunction(() => window.__game && window.__game.renderer && window.__game.sim, { timeout: 110000, polling: 500 });
await sleep(2500);
...
const built = await page.evaluate(() => {
  ...
  const p = sim.player;
  const roster = [
    ['Brightoak', 'druid', 1], ['Stormcaller', 'shaman', 1],
    ['Nightblade', 'rogue', 1], ['Emberlyn', 'mage', 1],
  ];
  const pids = roster.map(([name, cls], i) => {
    const pid = sim.addPlayer(cls, name);
    const e = sim.entities.get(pid);
    if (e) { e.pos = { x: p.pos.x + (i % 4) * 2 - 3, y: p.pos.y, z: p.pos.z + 2 }; e.prevPos = { ...e.pos }; }
    return pid;
  });
  const party = { id: sim.nextPartyId++, leader: me, members: [me, ...pids],
    raid: true, raidGroups: new Map(), lootStrategies: {} };
  party.raidGroups.set(me, 1);
  roster.forEach(([, , group], i) => party.raidGroups.set(pids[i], group));
  sim.parties.set(party.id, party);
  sim.partyByPid.set(me, party.id);
  pids.forEach((q) => sim.partyByPid.set(q, party.id));
...
console.log('raid tab text (before):\n' + await page.evaluate(() =>
...
console.log('raid tab text (after):\n' + await page.evaluate(() =>
  document.querySelector('#social-window .soc-body')?.innerText ?? '(no body)'));
await page.screenshot({ path: 'tmp/raid2party_after_panel.png' });
await clipSocial('tmp/raid2party_after.png');

console.log(built.raid && built.members === 5 && hasBtn && afterRaid === false ? 'CONVERT OK' : 'CONVERT FAIL');
await browser.close();
