...
// Minimal visible markers for gatherable world nodes (amber/heartwood/spore).
// Content and placements come from sim/content/gather_nodes.ts (merged into
// sim/data.ts's GATHER_NODES); this module only draws them. No harvest logic
// here (see PHAA-504); these are static, unowned fixtures.
//
...
const NODE_GEOMETRY: Record<GatherNodeType, () => THREE.BufferGeometry> = {
  amber: () => new THREE.IcosahedronGeometry(0.7, 0),
  heartwood: () => new THREE.ConeGeometry(0.55, 1.8, 6),
  spore: () => new THREE.BoxGeometry(0.5, 0.5, 0.5),
};
