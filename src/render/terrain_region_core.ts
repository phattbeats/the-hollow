...
// (x0, z0, size, spacing) and can use these helpers to decide which chunk
// geometries a sculpt region invalidates and which texel rows of a baked
...
// rebuild too or the seam shows a crack. Works for regular 60u chunks and
// merged far super-chunks alike (pass a larger size).
...
// a normal bake that reads a 1-texel derivative stencil should pass at least
// 1 so texels just OUTSIDE the sculpted region (whose stencil reaches inside
// it) rebake too. The floor/ceil mapping over-covers by up to one extra texel
// per side, which is safe (a rebake of an unchanged texel is a no-op). Returns
