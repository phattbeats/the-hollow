...
- `pages/`: one per route: `Overview`, `Usage`, `Accounts`, `Characters`, `Moderation` (+ `ModerationDetail`), `SuspiciousPlayers`, `AntibotConfig` (schema-driven bot-detector tuning; field labels are server data, chrome is `t()`), `SharedIps`, `ChatFilter`, `BlockedIps`, `BugReports`, `IpAssociations`, `Staff` (role management, gated `staff.manage`), plus the shared `AccountDetail`. `pages.ts` is the navigation tree (each item carries its `permission`; `visibleNavSections`/`firstVisiblePage` drive the sidebar filter and route-guard fallback).
...
GET: `/me`, `/overview`, `/provider-usage`, `/online`, `/suspicious-players`, `/antibot-config`,
`/antibot-config/history`, `/activity`, `/accounts?search&page`,
...
authorized against the declared permission map in `server/admin_routes.ts`. The `admin.*`
host is just routing, **not** security. `state/auth.svelte.ts` mirrors the token plus the
operator's roles/permissions for PRESENTATION only (sidebar filtering, the route guard in
`App.svelte`, hiding action buttons via `auth.can(...)`); the server re-checks every call.
`handleAuthFailure` logs out on **401 only**; a 403 means "missing permission" and
surfaces inline via `localizeAdminError`, never a logout. Never gate an action on
client-read state.
