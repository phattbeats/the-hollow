<script lang="ts">
  ...
  // grantable only via direct SQL), so superadmin rows and the operator's own
  // row render read-only; the server refuses both anyway.
      ...
      <tbody>
        {#each data.rows as row (row.accountId)}
          <tr>
            ...
            <td>
              {#if isSuperadmin(row)}
                <span class="hint">{t('staff.managedElsewhere')}</span>
              {:else if isSelf(row)}
      ...
      <tbody>
        {#each history.rows as row (row.id)}
          <tr>
            ...
            <td>{row.adminUsername ?? t('staff.viaDirectSql')}</td>
          </tr>
        {/each}
      </tbody>
    </table>
