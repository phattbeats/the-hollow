...
export class Input {
  ...
  private onKeyDown(e: KeyboardEvent): void {
    ...
    if (held === 'emoteWheel') {
      ...
      if (held === 'jump') this.keyJumpUntil = Math.max(this.keyJumpUntil, performance.now() + KEY_JUMP_LATCH_MS);
      this.noteIntent('move');
    }
  ...
  readMoveInput(): MoveInput {
    ...
    const jump = this.keybinds.codesForAction('jump').some((c) => this.keys.has(comboCode(c)))
