// Procedural adventure soundtrack, mostly pure WebAudio synthesis. Eastbrook
// keeps the original town theme intact. Every other zone pushes harder toward
// original old-school medieval MMO colors: lute/dulcimer loops, pipe/reed/
// square leads, wood-block ticks, frame drums, and faster dungeons. Each theme
// is a composed multi-track loop scheduled with a lookahead timer; zone
// changes crossfade. Two narrow recorded-file exceptions layer on top of the
// synthesis and duck it while active: the boss-fight loop (see
...
// The Hollow Reaches' hub (the shrine-gate clearing, PHAA-420) has no zone
// theme of its own today: it falls through to the plain 'vale' biome bed like
// any patch of open ground (Brandon's "Greenpaw's endless half-finished
// mix", PHAA-429/435). Once HUB_AMBIENT_TRACKS is populated, this zone id
// switches the hub over to the recorded ambient cycler instead; until then
// behavior is unchanged so shipping this is a zero-risk no-op.
const HOLLOW_HUB_ZONE_ID = 'the_hollow_reaches';
const HUB_AMBIENT_ZONE: MusicZone = 'hollow_hub';
const HUB_AMBIENT_CROSSFADE_SECONDS = 2.5;
const HUB_AMBIENT_VOLUME = 0.5;
...
//   hollow_hub  7 → A (shares the Hollow Reaches' 'vale' biome key; has no
//     procedural zone theme of its own, but combat can still reach the clearing)
...
export class MusicDirector {
  ...
  // biome-ignore lint/correctness/noUnusedPrivateClassMembers: read via an unsafe test cast (tests/music.test.ts) to clear the scheduler interval; never read in production code.
  ...
  // Hub ambient cycler (PHAA-435): recorded tracks played via plain <audio>
  // elements (like the boss loop, minus the AudioContext graph: this bed
  // never needs the lookahead scheduler), crossfading into one another and
  // ducking the procedural score while active.
  private hubAmbientActive = false;
  private hubAmbientEl: HTMLAudioElement | null = null;
  private hubAmbientCrossfadeStarted = false;
  private hubAmbientFadeTimer: number | null = null;
  ...
  // The dedicated Nythraxis track / hub ambient cycler owns the mix while active.
  private masterTarget(): number {
    if (!this._enabled || this._menuPaused || this.bossActive || this.hubAmbientActive) return 0;
  ...
  /** Engage/disengage the hub ambient cycler (PHAA-435). Idempotent; driven by
   *  update() below whenever the resolved zone crosses the hub boundary. Ducks
   *  the procedural score while active, same as the boss loop. */
  private setHubAmbientActive(on: boolean): void {
    if (on === this.hubAmbientActive) return;
    this.hubAmbientActive = on;
    if (on) this.startHubAmbient();
    else this.stopHubAmbient();
    if (this.ctx && this.master) {
      this.master.gain.setTargetAtTime(this.masterTarget(), this.ctx.currentTime, on ? 0.4 : 0.7);
    }
  }

  private hubAmbientTargetVolume(): number {
    return this.hubAmbientActive && this._enabled && !this._menuPaused
      ? HUB_AMBIENT_VOLUME * this._vol
      : 0;
  }

  private applyHubAmbientVolume(): void {
    if (this.hubAmbientEl) this.hubAmbientEl.volume = this.hubAmbientTargetVolume();
  }

  private startHubAmbient(): void {
    this.playHubAmbientTrack(pickNextHubAmbientTrack(HUB_AMBIENT_TRACKS, null));
  }

  private stopHubAmbient(): void {
    this.clearHubAmbientFadeTimer();
    if (!this.hubAmbientEl) return;
    this.hubAmbientEl.pause();
    this.hubAmbientEl = null;
    this.hubAmbientCrossfadeStarted = false;
  }

  private clearHubAmbientFadeTimer(): void {
    if (this.hubAmbientFadeTimer === null) return;
    window.clearInterval(this.hubAmbientFadeTimer);
    this.hubAmbientFadeTimer = null;
  }

  /** Starts `track`, crossfading out of whatever was already playing. Each
   *  track schedules its own successor near its end via `timeupdate`, so the
   *  cycle is self-sustaining once started; a single-track list just loops. */
  private playHubAmbientTrack(track: HubAmbientTrack | null): void {
    this.clearHubAmbientFadeTimer();
    const prevEl = this.hubAmbientEl;
    const prevVolume = prevEl?.volume ?? 0;
    this.hubAmbientEl = null;
    ...
    el.preload = 'auto';
    el.loop = HUB_AMBIENT_TRACKS.length <= 1;
    el.volume = prevEl ? 0 : this.hubAmbientTargetVolume();
    this.hubAmbientCrossfadeStarted = false;
    if (!el.loop) {
      el.addEventListener('timeupdate', () => {
        if (this.hubAmbientCrossfadeStarted) return;
        if (
          shouldStartHubAmbientCrossfade(el.currentTime, el.duration, HUB_AMBIENT_CROSSFADE_SECONDS)
        ) {
          this.hubAmbientCrossfadeStarted = true;
          this.playHubAmbientTrack(pickNextHubAmbientTrack(HUB_AMBIENT_TRACKS, track.id));
        }
      });
    }
    void el.play().catch(() => {
      /* needs a user gesture; retried on next hub entry/volume change */
    });
    this.hubAmbientEl = el;
    if (prevEl) {
      const target = this.hubAmbientTargetVolume();
      const steps = 24;
      let i = 0;
      this.hubAmbientFadeTimer = window.setInterval(
        () => {
          i += 1;
          const t = i / steps;
          el.volume = target * t;
          prevEl.volume = prevVolume * (1 - t);
          if (i >= steps) {
            this.clearHubAmbientFadeTimer();
            prevEl.pause();
          }
        },
        (HUB_AMBIENT_CROSSFADE_SECONDS * 1000) / steps,
      );
    }
  }
  ...
  update(zone: MusicZone, inCombat: boolean): void {
    ...
    const hubAmbientOn = zone === HUB_AMBIENT_ZONE && !inCombat && HUB_AMBIENT_TRACKS.length > 0;
    if (hubAmbientOn !== this.hubAmbientActive) this.setHubAmbientActive(hubAmbientOn);
    ...
    for (const layer of Object.values(this.layers)) {
      ...
      for (let guard = 0; guard < 220; guard++) {
        ...
        if (when >= ctx.currentTime - 0.03) {
          // biome-ignore lint/style/noNonNullAssertion: tickScheduler only runs on the init() interval, set up right after this.synth is assigned.
