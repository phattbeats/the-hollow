...
export function dealDamage(
  ...
  // Sourceless and self damage do not spend this enemy-hit defensive. Runs BEFORE
  // the WARFARE pvp multiplier so the restored HP is computed off raw incoming
  // damage (not the post-honor multiplier), matching damage.ts' absorb shield step.
...
export function handleDeath(ctx: SimContext, e: Entity, killer: Entity | null): void {
  ...
  if (e.kind === 'mob') {
    ...
    // World bosses (PHAA-494): snapshot the contributor set from the hate table
    // BEFORE it is cleared below, keep a long lootable-corpse window so every
    // contributor can loot, and never auto-respawn in place: the world-boss
    // scheduler is the sole respawner (it drops the corpse once the window
    // elapses). Summoned adds collapse with the boss: leaving them alive would
    // harass looters for the whole window, and a slain add's in-place respawn
    // timer would revive it mid-window (only fires for worldBoss templates, so no
    // parity rng change).
