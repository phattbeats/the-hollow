...
//
// This module follows the items.ts pattern (src/sim/CLAUDE.md): pure capacity/
// stacking math a Vitest imports directly, plus the two command bodies
// (equipBag/unequipBag) as free functions `fn(ctx, ...)` behind SimContext.
// Backing state stays on Sim (PlayerMeta.bags); Sim keeps thin same-named
// delegates.
//
