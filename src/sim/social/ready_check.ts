...
// event), and the leader is auto-marked ready. Members answer yes/no; anyone
// still pending READY_CHECK_SECONDS later is marked "no response". The outcome
// is announced to the whole party as chat/log lines. State lives on Sim as
// `ctx.readyChecks` (party id -> ReadyCheck); this module owns the logic.
// Draws NO rng, uses the sim clock (ctx.time) not wall-clock, so replays and
// the parity gate stay byte-stable.
import type { SimContext } from '../sim_context';
import type { ReadyCheck } from '../types';

// Classic-era ready checks give stragglers 30 seconds; anyone still pending
// then is tallied as "no response".
...
export function readyCheckStart(ctx: SimContext, pid?: number): void {
  ...
  for (const mPid of party.members) {
    // The initiator is auto-ready (and gets no prompt), mirroring the classic
    // client.
