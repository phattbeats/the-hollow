...
//
// ADAPT (PHAA-573), Tier 2: ported from upstream #1613 onto this fork's Ashen
// Coliseum arena framework, the boarball (PHAA-572) precedent for every seam.
// The upstream daily-reward hook is DROPPED per the ticket (same ban as Vale
// Cup: docs/plan-the-hollow.md:412, PHAA-518 SKIP); it was only an exclusion
// guard returning 0 arena-reward points for yumi formats, and this fork has no
// daily_rewards.ts landed, so there is no code path here to remove: yumi never
// touches currency and endArenaMatch treats it as unranked exactly like fiesta.
