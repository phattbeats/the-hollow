...
// NYTHRAXIS_BOSS_ID relocated to types.ts (I1: shared by the dungeon raid-door seal);
// imported above. The Nythraxis encounter sites here still read it.
...
// Exported for social/chat.ts (broadcastEmote); chat()'s /roll + say/yell ranges
// also read it. /say carries a short distance; /yell across a camp.
...
// TRADE_RANGE moved to social/trade.ts with the trade methods.
...
// PET_TELEPORT_DISTANCE relocated to types.ts (shared by Sim.updatePet + I2c companion AI).
...
export class Sim {
  ...
  private buildSimContext(): SimContext {
    ...
    const host: SimContextHost = {
      ...
      // L2: the World Market escrow (marketList) pulls the stack from the seller's
      // bags via removeItem; the inventory hub stays on Sim (points-at Sim).
      removeItem: (itemId, count, pid) => sim.removeItem(itemId, count, pid),
      ...
      // I2c delve companion AI now lives in src/sim/delves/companion.ts; the mob-AI
      // dispatch (updateMob, owned non-stunned companion branch) calls it via
      // ctx.updateDelveCompanion (late-bound -> companionMod). It reaches the shared
      // mob/movement/hostility helpers (all still on Sim) through these: mobSwing +
      // moveToward are shared entry points; isHostileTo/isRooted/moveSpeedMult/
      // swingIntervalMult stay on Sim. (M2/T1/C3 register the same six with identical
      // signatures on the integrated base -> dedupe to one each at integration; keep
      // updateDelveCompanion's points-at = delves/companion.)
      ...
      // G2 social plumbing: hasPendingSocialInvite is the shared invite gate (A2
      // also registers it on the integrated base -> dedupe); setPlayerLevel backs
      // the /dev level cheat in social/chat.ts; notice is the /join /leave chat-log
      // line. All stay on Sim. (tradeFor above now delegates to social/trade.ts.)
  ...
  private updateMob(mob: Entity): void {
    ...
    if (mob.ownerId !== null) {
      ...
      if (this.isDelveCompanionMob(mob)) {
        this.ctx.updateDelveCompanion(mob);
        return;
      }
      this.updatePet(mob);
      return;
