...
export class Sim {
  ...
  private buildSimContext(): SimContext {
    ...
    const host: SimContextHost = {
      ...
      // finishes building it). despawnPersistentPet + clearNonPlayerStatAuras stay
      // Sim methods, exposed here for the moved respawnMob to consume.
      ...
      despawnPersistentPet: sim.despawnPersistentPet.bind(sim),
      ...
      // P1a pet-AI seam: the helpers the moved updatePet/petRangedAttack/petPickTarget
      // reach back for. All STAY on Sim (syncPetAspect is pet-management, P1b owns it;
      // effectiveAttackPower scales the imp bolt; isHostileTo is the shared predicate).
  ...
  private handleDeath(e: Entity, killer: Entity | null): void {
    ...
    if (e.kind === 'mob') {
      ...
      this.ctx.frenzyPackmates(e); // wild packmates fly into a frenzy when one falls
      this.ctx.armDeathThroes(e); // volatile corpses begin to destabilize, then burst
