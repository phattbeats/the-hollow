...
export class Sim {
  ...
  private buildSimContext(): SimContext {
    ...
    const host: SimContextHost = {
      ...
      addItem: sim.addItem.bind(sim),
      // L1 loot quest-drop branch (loot/loot_roll.ts needsQuestDrop) reads it; the
      // inventory hub stays on Sim. Q1/L2 register the same sig on their tracks ->
      // dedupe to one (points-at Sim) at integration.
      ...
      // C5 auto-attack surface: shared helpers the moved swing driver consumes, both
      // STAY on Sim (aggroMob has ~20 callers; swingIntervalMult is a stat read). M2
      // registers identical sigs on the Mob track -> dedupe to one each at integration.
