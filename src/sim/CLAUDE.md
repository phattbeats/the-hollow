...
The simulation used to be a single ~17.5k-line `Sim` class. It has been refactored so
each self-contained game SYSTEM lives in its own sibling module, and `sim.ts` is now a
...
- The refactor was a MOVE, not a rewrite: behavior is byte-identical, proven by the
...
- **Never import from `render/`/`ui/`/`game/`/`net/` or anything DOM/Three** — this module must run unchanged in Node. (Sibling modules are `src/sim`-pure too, enforced by `tests/architecture.test.ts`.)
...
- **Never reach past `SimContext` from a module back into `Sim` internals**, and never rename or repurpose a `SimContext` callback (the seam is append-only).
