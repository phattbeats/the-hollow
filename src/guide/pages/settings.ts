// Settings & Performance reference: three ready-made graphics loadouts (best FPS,
// balanced, best visuals) plus a plain-language tour of every graphics option.
// Setting and value labels reuse the game's own hud.options.* / hudChrome.* /
// game.settings.* keys so the wiki always matches the in-game Options menu in
// every locale; only the guide prose is new.
//
// Every gameplay-facing claim on this page mirrors
// `docs/design/graphics-settings-fairness.md`: the option names, the tier
// composition, reload semantics, first-run detection, the runtime safety net
// that runs on every tier below Ultra, the mobile high-tier cap, and the
// nothing-here-is-gameplay-affecting statement all reflect the code
// (src/game/settings.ts, src/render/gfx.ts, src/game/browser_env.ts, and the
// ui_effects_profile tier knobs). No engine jargon or internal knob values are
// exposed.
