 ...
 * out-of-range flag (computed inline, identically to the selector), level, and the
 * active title id (PHAA-748: a deliberate setActiveTitle flips this), plus the leader,
 * raid flag, and the player's own group. The player is skipped (the frames never show
 * the local player), matching the selector's `pid !== playerId`.
 *
 ...
 * (a field, a join/leave, an out-of-range flip, a title change) changes this string,
 * and nothing the selector depends on is omitted, so an equal signature means an
 * identical render.
 */
export function partyFrameSignature(
  ...
  for (const m of info.members) {
    ...
    sig +=
      ...
      `${m.dead}:${m.inCombat}:${oor ? 1 : 0}:${m.level}:${m.atitle ?? ''}|`;
  }
