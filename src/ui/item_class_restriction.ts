...
// restrictions armor does not actually enforce (see classesThatCanEquipArmorType).
...
// Mirrors the sameClassSet helper in equipment_rules.ts but takes an array (the
// classesThatCanEquipArmorType result), since the helper there takes a Set.
function sameClassList(classes: readonly PlayerClass[], allowed: readonly PlayerClass[]): boolean {
...
export function requiredClassesForTooltip(item: ItemDef): readonly PlayerClass[] | null {
  ...
  if (armorType) {
    return sameClassList(item.requiredClass, classesThatCanEquipArmorType(armorType))
      ? item.requiredClass
      : null;
  }
