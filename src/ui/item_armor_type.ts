...
// armor type an item is lives in the sim's armorTypeForItem (explicit field, else
// inferred from requiredClass); this leaf only turns that into a label key so the
// HUD consumer stays a thin lookup. DOM-free and i18n-runtime-free, unit-tested in
// tests/item_armor_type.test.ts.
import { type ArmorType, armorTypeForItem } from '../sim/equipment_rules';
...
// Returns the label key for the item's armor subtype, or null when the item has no
// resolvable armor type (non-armor items, or generic armor with no class hint).
