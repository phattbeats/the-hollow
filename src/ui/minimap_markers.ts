...
export type MinimapMarker =
  ...
  // A gatherable world node (amber/heartwood/spore, PHAA-508): `ready`
  // distinguishes harvestable-for-THIS-viewer from on-cooldown-for-this-viewer
  // (per-player, see IWorldGathering#nodeHarvestableByMe; two viewers can see
  // opposite states for the same node id).
...
export function createMinimapMarkers(): MinimapMarkers {
  ...
  return {
    build(world: IWorld, S: number, pxPerYard: number): MinimapModel {
      ...
      // Gatherable world nodes (PHAA-508): static content positions (never entities), each
