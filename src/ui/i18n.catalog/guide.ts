...
export const guideStrings = {
  ...
  home: {
    ...
    title: 'The Hollow',
    subtitle:
      'Walk the Reaches, hear the Plant, and tend the shrine it is bound to. Quest, group up, and grow your own corner of a hand-built world, free in your browser.',
    ctaPlay: 'Play Now',
    ...
    what: {
      ...
      pillarPlayBody:
        'No download, no launcher. Make a character and you are at the Gate in seconds, on desktop or phone.',
    ...
    // World teaser. The home card grid leads with The Hollow Reaches
    // (the new sealed-onboarding zone, levels 1-4, hub The Hollow Gate)
    // and then walks the road north into the three open biomes.
    world: {
      ...
      hollowReachesBlurb:
        'Open ground and old farm plots at the threshold of a sealed shrine. The Plant keeps the lamp lit; the road waits beyond the gate.',
  ...
  classPage: {
    ...
    professionsNote:
      'Any class above can also be taken as a profession, a secondary class that opens a second talent tree without dropping your primary role. See how professions work before you pick.',
  ...
  questsPage: {
    ...
    // The Plant-god arc, teased as a trail that runs out of the Hollow and
    // along the road. Spoiler-safe: no boss names, no chapter endings, only
    // the shape of the thread and the people who hand you the next strand.
    sagaTitle: 'From the shrine, into the wood',
    sagaBody:
      "The main thread of the story starts in the Hollow Reaches, at the vase, and follows the road north. The Plant keeps the shrine's lamp lit, and the friends who keep it standing pass the trail on to the next. You never face the whole weight of it at once; you take one step, and it shows you the next.",
    sagaReachesTitle: 'The Reaches: a lamp and a calendar',
    sagaReachesBody:
      'In the Hollow Reaches, Brother Greenpaw keeps the shrine vase alive with emberbulbs, and the verger and the sexton set the planting by the moon. The first thread is a small one: keep the lamp lit, clear the wolves, learn the ground. By the time the road opens, you will know whose hand the lamp is in.',
    sagaValeTitle: 'The Vale: the lamp at the gate',
    sagaValeBody:
      'Eastbrook is the first town you reach on the road north. The marshal there knows the Plant by name, even if the town is slow to say it. Stand the watch, hunt the cultists who would have the lamp out, and the marshal will point you to the marsh and to the warden who holds the bridge there.',
    sagaMarshTitle: 'The marsh: a hand of drowned roots',
    sagaMarshBody:
      'In Mirefen, the water has begun to remember the Plant in a wrong way. Someone is filling the fen like a tithing box, growing obedient dead from every traveler the water takes, and the warden at the bridge has been holding them at the gate as long as her arms allow. Chase the orders up the chain to the drowned bastion, and you will learn whose roots are reaching up from beneath the peaks.',
    sagaPeaksTitle: 'The peaks: what the roots were for',
    sagaPeaksBody:
      "On Thornpeak the whole scheme comes clear. Every soul drowned since the Vale was a tithing poured toward the cult's grim work in the mountain's heart, and the wall at Highwatch is all that has held the lamp up to now. The trail that began at a vase ends here, in a ten-player descent to face the hand behind it all. We will let you find out what waits at the bottom.",
  ...
  // Recurring characters and in-world voices, shared across the World and
  // Quests pages. The Hollow Reaches casts a Verger and a Sexton who keep
  // the lamp lit and the calendar straight; beyond them, the road north
  // still passes through Eastbrook, Fenbridge, and the wall at Highwatch,
  // so the marshal/warden/captain voices in worldPage stay where they are.
  lore: {
    figuresTitle: 'Faces you will come to know',
    figuresBody:
      'Begin at the shrine: the sexton and the verger who keep the Reaches to a calendar. The road north will hand you the marshal, the warden, and the captain in their turn.',
    zebediahRole: 'Verger of Root Hollow',
    zebediahBody:
      'A patient verger who keeps the Reaches to a calendar, his hands always half in the soil. He watches the lamp at the vase, sets the planting by the moon, and is the first voice the road speaks to a new arrival.',
    faddickRole: 'Sexton of Fallow Acres',
    faddickBody:
      'A quiet sexton with a long-handled tool, more at home with a spade than a sword. He clears what creeps in from the wood, lays the dead to rest, and remembers every plot on his ground. When the trail leaves the shrine, it is often him who points the way.',
  },
  ...
  // The multiclass / secondary-profession system (GW1 build, Phase 3, PHAA-466).
  // Spoiler-safe: names only the level the gate opens, the shared-pool half cap, the
  // escalating respec gold cost tiers, and that the trainer NPC lives in the hub town.
  // No balance numbers, no exact copper amounts, no tree names. Mirrors the in-game
  // "How do secondary professions work?" copy in questUi.dialog.trainerHowBody so the
  // guide and the trainer panel say the same thing.
  professionsPage: {
    heading: 'Professions and multiclassing',
    intro:
      'A profession is a second class you train alongside your primary one, opening a second talent tree and a slice of its ability kit without leaving your first role behind.',
    whatHeading: 'What a profession is',
    whatBody:
      "Every class can be taken as a profession by any other class. Pairing one adds a second talent tree and lets you borrow a few of its abilities, so a Warrior can dabble in the Priest's healing or a Mage pick up a Rogue's sting. Your primary class still sets your role, your resource, and your identity; the profession bends the build without replacing it.",
    howHeading: 'How to pick one',
    howBody:
      'Professions open at level 10, the same moment your first talent tree unlocks. Visit a Profession Trainer in the hub town, open their dialog, and pick the class you want as your secondary.',
    costBody:
      'Your first pick is free. Changing to a different class later costs gold, and the fee steps up each time you swap, so an early decision is never punished while later reshuffles carry a small price.',
    resetTitle: 'Nothing is locked in',
    resetNote:
      'You can refund points in either tree any time you are out of combat, and you can swap your secondary class at the same trainer for an escalating gold fee. An early profession pick is a first draft, not a commitment.',
    ...
    trainersBody:
      'A Profession Trainer stands in the hub town near the shrine. Any single trainer can teach you any class as a profession (other than your primary), so one visit is enough to set or change your secondary.',
  },
