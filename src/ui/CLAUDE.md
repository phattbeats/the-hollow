...
- **`i18n.catalog/`** is the authoritative English source catalog (nested domain modules
  ...
  barrel) that drives `TranslationKey` (the build-generated flat union in
  `translation_keys.generated.ts`, regenerated by `npm run i18n:gen`), the dotted-path type
  every `t()` uses. Add a new English string in the matching domain module.
