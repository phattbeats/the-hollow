  ...
  "entities": {
    ...
    "quests": {
      ...
      "q_fallow_acres_wolves_ii": {
        ...
        "text": "По краям поредело, а логово нет. Ещё восемь, и Перелоги смогут держать крышу, не теряя ночами тех, кто спит под ней. Я бы приглядел сам, да только дважды в одном месте бываю редко, а земля под Окраинами просит присмотра больше, чем эти борозды.",
    ...
    "dungeons": {
      ...
      "under_shrine": {
        "name": "Подсвятилище",
        "enterText": "Ты спускаешься под святилище. Воздух становится неподвижным и тесным, и тьма впереди не кажется пустой. Что-то здесь, внизу, всё ещё ведёт свой счёт времени, хотя его давно никто не слушает.",
        "leaveText": "Ты выбираешься обратно в тепло. Над тобой, едва заметно, дым. А внизу тьма продолжает свой медленный счёт."
      }
    },
    "delves": {
      "collapsed_reliquary": {
