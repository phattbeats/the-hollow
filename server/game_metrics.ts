...
// (server/metrics.ts). Prometheus attaches env / service=game / server_name at
...
// the process-wide slot in server/game_signals.ts (installed by main.ts at boot
// with the sink this function returns), exactly like the attack-signal counters.
//
// CARDINALITY IS BOUNDED BY DESIGN, same contract as server/metrics.ts: the only
// label values are the fixed tick-phase names, the two per-phase stats (p95, max),
// and the two ws directions (in, out). Nothing per-player (account id, character
// id, name, ip) is ever a label. The tick-phase series count is fixed at
