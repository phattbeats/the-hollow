// An in-house, pino-shaped structured JSON logger for the server. Ported from
// upstream's server/http/logger.ts (levy-street/world-of-claudecraft#1491,
// primitive 5/6 of the PHAA-519 REST decomposition), minus the request-scoped
// AsyncLocalStorage reqId: our server has no request-context store, so lines
// carry only their explicit fields. No dependency (pino is deliberately
// forbidden here); it is a thin facade over process.stdout / process.stderr
// that writes EXACTLY ONE JSON object per line.
//
// Shape of every emitted record: { level, time (epoch ms), msg, ...child
// bindings, ...call fields }. The whole record passes through redact()
// (server/log_redact.ts) before JSON.stringify, so a secret in a bound field or
// a caller's fields can never reach the log. info goes to stdout; warn and
// error go to stderr.
//
// The logger NEVER throws: a logging failure (a circular structure redact could
// not break, a serializer error) is swallowed so it can never crash the request
// path. The transport is injectable (createLogger({ out, err })) for
// deterministic tests; the default exported singleton is bound to the process
// streams.
//
...
// Bearer + 64-hex + email shapes, so a raw URL or header blob carrying a
// non-hex secret (a PKCE code_verifier, a base32 TOTP secret, an OAuth state)
// relies on the caller not logging it wholesale. Pass an Error as a TOP-LEVEL
...
/**
 * Build one JSON line and hand it to `write`. Never throws: a serialization
 * failure is swallowed so logging cannot break the request path.
 */
function emit(
