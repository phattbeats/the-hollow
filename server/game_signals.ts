...
// the character-create path in main.ts) threading a sink through its
// constructors. main.ts installs the real implementation
// (registerGameStateMetrics(...), so all three counters share the exporter's one
// registry) once at boot, exactly like setAttackSignalSink (server/attack_signals.ts);
// before that, and in any test that never wires one, the slot holds the no-op and
...
// GameStateSource the exporter registration captures. See server/game_metrics.ts.
//
// CARDINALITY IS BOUNDED BY DESIGN, same contract as server/metrics.ts: the only
// label value here is the ws-message direction, one of a fixed two. Nothing
