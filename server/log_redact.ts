...
// (server/logger.ts). Ported from upstream's server/http/redact.ts
// (levy-street/world-of-claudecraft#1491, primitive 5/6 of the PHAA-519 REST
// decomposition). It scrubs the known secret and PII classes out of any value
// BEFORE it is serialized to a log line, so an operational log can never carry
// a live credential:
...
//   (g) raw byte values (Buffer / TypedArray / ArrayBuffer) collapse to the
//       placeholder wholesale, so a secret held as bytes under a non-secret key
//       name can never serialize into a line;
//   (h) email addresses (local@domain.tld) anywhere in a string, since signup
//       and recovery flows carry raw addresses that must never land in a log.
//
// Defensive by construction: it recurses into nested objects and arrays with
...
// non-secret fields verbatim (Dates and numbers pass through untouched), and
// NEVER throws: a logger must not crash a request, so any internal failure
// collapses to a safe placeholder.
//
 ...
 * regardless of the value's type. Deliberately does NOT include a bare 'code':
 * the OAuth authorization/device code VALUES are 64-hex (caught by the value
 * patterns), and a blanket 'code' rule would wrongly scrub short machine codes.
 */
const SECRET_KEY_NEEDLES: readonly string[] = [
  ...
  'secret', // covers 'client_secret', 'pendingSecret', 'totpSecret', and *_key variants
 ...
 * machine code under a 'code' key survives while a 6-digit TOTP code or a
 * dashed user code does not.
 */
...
// An email address (local@domain.tld) anywhere in a string. Deliberately RFC-lite
// and conservative: it requires a dotted TLD of 2 to 24 letters, so a bare
// '@handle' or a 'name@build' version tag (no dot-TLD) survives while a real
// address is redacted. BOUNDED quantifiers (the RFC 5321 caps: 64-char local
// part, 255-char domain) keep the per-position backtracking constant, so a
// non-matching adversarial value scans in time linear in its length; the
// redactor runs on the same event-loop thread as the 20 Hz world loop, so an
// unbounded (quadratic) form is not acceptable here.
 ...
 * placeholder (no infinite loop), while a value merely referenced twice in
 * sibling positions is still fully redacted each time (which keeps the result
 * idempotent).
 */
