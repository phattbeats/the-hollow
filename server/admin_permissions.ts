...
// closed set consumed by code (the route table in admin_routes.ts, tests);
// operators are never assigned permissions directly. Roles are job-shaped
// bundles; an account carries a set of roles (accounts.admin_roles) and its
// effective permissions are the union. Trimmed to the routes this fork's
// admin.ts actually serves (no map-editor content moderation, tick-loop
// profiling, or in-game spectate command, which upstream also gates here).
...
// superadmin is grantable only via direct SQL, never from the dashboard; the
// staff API refuses it (see ASSIGNABLE_ADMIN_ROLES).
...
// Strict validation for role WRITES (the staff API): null on any non-array or
// unknown role name, deduped, normalized to vocabulary order.
