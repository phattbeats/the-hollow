...
// (server/metrics.ts), exactly like the game-state half (server/game_metrics.ts).
// Prometheus attaches env / service=game / server_name at scrape time, so nothing
// here emits those.
//
...
// aggregate, so a PeriodicCollector (server/periodic_collector.ts) runs the ONE
// overviewCounts() batch on a 30-60s interval and caches it; the gauges publish the
// cached snapshot when registry.metrics() runs. A scrape never hits the DB, so a
...
// label is `window`, drawn from a fixed small set (today/week/month, all_time).
