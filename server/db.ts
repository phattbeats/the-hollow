...
-- so it migrates a genuine legacy admin exactly once and then no-ops forever.
-- Legacy admins are migrated to the admin role (the full toolset MINUS
-- staff.manage), not superadmin: staff-role management requires a deliberate
-- superadmin grant (direct SQL; there is no grant script in this fork).
...
-- Audit trail for staff role changes (dashboard Staff page).
