...
// This fork's placementsToRenderAssets takes an injected resolver (no baked-in
// catalogue, see custom_map.ts); these tests only exercise user-upload ids and
// unknown ids, so resolving through userAssetPath alone is enough.
const resolveAssetPath = (assetId: string) => userAssetPath(assetId) ?? undefined;
