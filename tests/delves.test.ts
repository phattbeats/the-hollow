...
describe('delve reward chest + surface exit flow', () => {
  function enterFinale(sim: ReturnType<typeof makeSim>) {
    ...
    // the ultra-rare roll (some seeds happen to roll Bountiful). The Bountiful-Coffer
  ...
  it('the Bountiful roll is deterministic for a given seed', () => {
    ...
    // ⇒ same outcome; seed 203 is known to roll Bountiful (the fixtures below pin
    // run.bountiful explicitly, so they don't depend on this specific seed).
