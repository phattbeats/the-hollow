...
// This fork has no baked-in asset catalogue inside custom_map.ts (see its module
// comment): every resolution takes an injected resolver. These tests never
// exercise resolvable placements, so a resolver that always misses is enough.
const NO_ASSET_CATALOG = () => undefined;
...
describe('CustomMap build + projection', () => {
  ...
  it('projects to a WorldContent the engine can consume', () => {
    ...
    const w = customMapToWorldContent(map, NO_ASSET_CATALOG);
