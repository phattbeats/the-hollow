...
// it lands the matching edit here in the SAME commit. The count pins (202 / 60 / 142)
...
// The 33 facet interfaces the W1 split produced (src/world_api/<facet>.ts). Imported
// type-only to pin each facet's runtime member array to its interface key-set below.
import type { IWorldAchievements } from '../src/world_api/achievements';
import type { IWorldChat } from '../src/world_api/chat';
import type { IWorldCollections } from '../src/world_api/collections';
...
import type { IWorldHomestead } from '../src/world_api/homestead';
...
import type { IWorldTrainer } from '../src/world_api/trainer';
...
export const IWORLD_MEMBERS = [
  ...
  { name: 'deedLog', kind: 'data' },
  { name: 'deedsDone', kind: 'data' },
  { name: 'earnedTitles', kind: 'data' },
  ...
  // --- Crafting (PHAA-574): craft a recipe + the local viewer's own craft
  ...
  // PHAA-641: the readyrespond command (a UI button click, not chat text); the
  // ready check itself starts via the "/ready" chat command, needing no member.
  ...
  // --- Dungeon Finder solo-role queue (PHAA-736, phase 1) ---
  ...
  // --- Collection tracking core (PHAA-625/626) ---
  { name: 'collectedIds', kind: 'data' },
  { name: 'readCollectible', kind: 'method' },
  // --- Achievements (PHAA-687) ---
  { name: 'unlockedAchievementIds', kind: 'data' },
  { name: 'achievementPoints', kind: 'data' },
...
// --- W1: aggregate == disjoint union of the 33 facet member sets --------------------
...
//   (3) the 33 arrays are pairwise DISJOINT (a member filed in two facets reddens);
...
const FACET_TRAINER = [
  ...
  'secondaryClassCost',
  'setSecondaryClass',
] as const satisfies readonly (keyof IWorldTrainer)[];
type _ExhaustTrainer = AssertNever<Exclude<keyof IWorldTrainer, (typeof FACET_TRAINER)[number]>>;
...
const FACET_GREENPAW_HEARTH = [
  'hollowHearth',
  'feedGreenpaw',
] as const satisfies readonly (keyof IWorldGreenpawHearth)[];
type _ExhaustGreenpawHearth = AssertNever<
  Exclude<keyof IWorldGreenpawHearth, (typeof FACET_GREENPAW_HEARTH)[number]>
>;

const FACET_HOMESTEAD = ['homesteadInfo'] as const satisfies readonly (keyof IWorldHomestead)[];
type _ExhaustHomestead = AssertNever<
...
const FACET_COLLECTIONS = [
  'collectedIds',
  'readCollectible',
] as const satisfies readonly (keyof IWorldCollections)[];
type _ExhaustCollections = AssertNever<
  Exclude<keyof IWorldCollections, (typeof FACET_COLLECTIONS)[number]>
>;

const FACET_ACHIEVEMENTS = [
  'unlockedAchievementIds',
  'achievementPoints',
] as const satisfies readonly (keyof IWorldAchievements)[];
type _ExhaustAchievements = AssertNever<
  Exclude<keyof IWorldAchievements, (typeof FACET_ACHIEVEMENTS)[number]>
>;

// The 33-facet partition, keyed by facet for legible failure messages.
const FACET_MEMBER_ARRAYS: Readonly<Record<string, readonly string[]>> = {
  ...
  greenpawHearth: FACET_GREENPAW_HEARTH,
  homestead: FACET_HOMESTEAD,
...
describe('W1: aggregate IWorld member set equals the disjoint union of the 33 facets', () => {
  it('pins the facet count at 33', () => {
  ...
  it('the 33 facet arrays are pairwise disjoint (no member filed in two facets)', () => {
  ...
  it('the union of the 33 facets equals the pinned 202-member IWORLD_MEMBERS set', () => {
