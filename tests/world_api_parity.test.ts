...
// it lands the matching edit here in the SAME commit. The count pins (179 / 49 / 130)
...
// The 179 members of `interface IWorld`, in interface order (world_api.ts).
// Partition: 49 `data` + 130 `method` (read-returning + command-void + 3 async).
...
describe('IWORLD_MEMBERS is the pinned IWorld contract (anti-loosening)', () => {
  it('pins total / data / method counts', () => {
    expect(IWORLD_MEMBERS.length).toBe(179);
  ...
  it('the full sorted member set is exactly the pinned 179', () => {
...
//   (4) their union, sorted, equals the pinned 179-name IWORLD_MEMBERS set (a member
...
describe('W1: aggregate IWorld member set equals the disjoint union of the 26 facets', () => {
  ...
  it('the union of the 26 facets equals the pinned 179-member IWORLD_MEMBERS set', () => {
    const union = Object.values(FACET_MEMBER_ARRAYS).flatMap((arr) => [...arr]);
    expect(union.length, 'union size before dedup (catches a duplicated member)').toBe(179);
    expect(new Set(union).size, 'union size after dedup (catches a duplicated member)').toBe(179);
