...
describe('npcs', () => {
  ...
  it('preserves the banker flag through the sanitizer (PHAA-571 bank anchor)', () => {
    const doc = sanitizeMapDoc(
      rawDoc({ npcs: { b: { id: 'b', pos: { x: 0, z: 0 }, banker: true } } }),
    );
    expect(doc?.content.npcs.b?.banker).toBe(true);
    const noBanker = sanitizeMapDoc(rawDoc({ npcs: { n: { id: 'n', pos: { x: 0, z: 0 } } } }));
    expect(noBanker?.content.npcs.n?.banker).toBeUndefined();
  });
});
