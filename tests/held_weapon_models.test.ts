...
describe('held weapon models', () => {
  ...
  // fallback (per PHAA-587): the chibi rig's DEF-hand.R/L bones don't match the
  // KayKit handslot.r/l data an attach needs, so a Blender-authored chibi grip
  // table is follow-up work (PHAA-583), not yet built. The cosmetic Combat Mech
  // (player_mech) is class-agnostic but is included: it still shows the wearer's
  // equipped mainhand, like every other KayKit-rig body.
  it('all player classes swap the mainhand except the hunter and chibi _f classes', () => {
    ...
    for (const key of players) {
      ...
      if (key === 'player_hunter') {
        ...
        expect(
          def.weaponSlots,
          `${key} is chibi-rigged, no held weapon yet (PHAA-587)`,
        ).toBeUndefined();
      } else {
