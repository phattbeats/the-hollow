...
// This fork has no generated asset-id -> path catalogue yet (see custom_map.ts
// header), so placementsToPlayAssets takes an injected resolver; this stub
// stands in for the real catalogue/upload lookup the client shell will supply.
const resolveAssetPath = (assetId: string): string | undefined =>
...
describe('effectiveCollideRadius', () => {
  ...
  it('flows into the playtest PlacedAssets (both paths read the same record)', () => {
    ...
    const [walkThrough] = placementsToPlayAssets(
      [{ ...base, collide: false, collideRadius: 7.5 }],
      resolveAssetPath,
    );
