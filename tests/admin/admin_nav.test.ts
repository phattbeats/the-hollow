...
describe('AdminNav', () => {
  ...
  it('filters sections and items down to the granted permissions', () => {
    ...
    // Moderation section: reports/shared-ips/blocked-ips/chat-filter (all
    // moderation.read) stay; suspicious-players/antibot-config (bot-detector
    // permissions) do not.
    ...
    expect(screen.getByRole('link', { name: t('nav.chatFilter') })).toBeInTheDocument();
    expect(
      screen.queryByRole('link', { name: t('nav.suspiciousPlayers') }),
    ).not.toBeInTheDocument();
    expect(screen.queryByRole('link', { name: t('nav.antibotConfig') })).not.toBeInTheDocument();
    ...
    // analytics.read, Operations/Usage needs ops_usage.read, neither granted.
