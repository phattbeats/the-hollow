// PHAA-506 (upstream #1190/#1122): proficiency-scaled material rarity roll on
// node harvest. Ports upstream's tests/professions_rarity_roll.test.ts onto
// this fork's per-node-type proficiency model (Record<GatherNodeType, number>,
// PHAA-505), including the two upstream hardening fixes: the NaN-proficiency
// common-only clamp (762dd1945) and the rng draw-order / tuning-constant pins
// (e23d89535).
...
describe('material rarity roll (PHAA-506)', () => {
  ...
  it('a denied harvest never draws rng (the roll happens only on a grant)', () => {
    ...
    // needed for this leaf-level pin.
    const meta = {
      gatheringProficiency: { amber: 50, heartwood: 50, spore: 50 },
    ...
    expect(granted.rarity).toBeDefined();
    expect(draws).toBe(1);
  });
