...
// (server/game.ts) and the exporter registration (server/game_metrics.ts): the
// gauges reflect real joined sessions/accounts/entities at scrape time, and the
...
// (server/game_signals.ts). The exporter's own unit tests (tests/game_metrics.test.ts)
// pin the exposition shape; this file pins that the GameServer actually feeds it.

import { Registry } from 'prom-client';
import { afterEach, describe, expect, it, vi } from 'vitest';

// Mock the db layer so no Postgres is needed (mirrors tests/chat_log.test.ts).
vi.mock('../server/db', () => ({
...
import { type GameStateSource, registerGameStateMetrics } from '../server/game_metrics';
import { noopGameMetricsCounters, setGameMetricsCounters } from '../server/game_signals';
