...
## Current status
- **Active phase:** Phase 3 DONE (flatten main-table overlays); next up Phase 3 QA, then Phase 4. Phase 2 DONE (commits `3f1ed8d`, `a92ff37`, `ffb40e5`); Phase 2 QA still NOT STARTED. Phase 1 DONE + QA PASS.
...
- PHASE 3 HANDOFF TO PHASE 4 (islands): the two island files were intentionally left NESTED in Phase 3 (see additions-log row 3). After full-inline of the main-table overlays, `world_entity_i18n.ts`'s non-English slices have NO consumer (the locale files no longer spread them; zero runtime path-indexers; only `.en` feeds nested `en`) - they are dead/superseded source. Phase 4 should decide: flatten-and-keep as the single entity-name source, or remove the now-dead non-English data. `talent_i18n.ts` stays a separate function-valued channel (not flattenable to a string map; `tTalent` reads it nested at runtime) - Phase 4/later must keep it nested or re-architect the formatter, not naively flatten it. The state.md/phase-doc line "the island files are flattened too" did NOT happen and was a wrong assumption about their shape.
