...
## Current status
- **Active phase:** Phase 2 (Phase 1 COMPLETE 2026-06-17; **Phase 1 QA COMPLETE-WITH-FOLLOWUPS 2026-06-18**, verdict PASS-WITH-FOLLOWUPS, 0 BLOCKING - the resolved table is now a generated DIRECTORY + barrel, SHA invariant, bundle-neutral). On 2026-06-18 the branch merged `release/v0.10.0` (merge 5e78a85) and baselines were re-measured against v0.10. The v0.10 main-chunk gzip is **1,263.10 kB** (measured during QA; supersedes the stale ~1.19 MB pre-v0.10 figure). Phase 1 QA added `tests/i18n_emit_shape.test.ts` (pins the barrel/`LOCALE_LOADERS`/`SUPPORTED_LANGUAGES`/`pending` surface for game+admin + an `I18N_OUT_DIR` determinism/orphan-sweep check), hardened both build scripts' orphan-sweep to also prune `*.ts.tmp` crash leftovers, swept stale single-file-path comments, documented the intentional admin-no-SHA asymmetry, and removed dead `fix_braces.mjs`.
