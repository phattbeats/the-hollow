...
## Branch / integration
All code work runs on `feature/i18n-lazy-locales` (originally cut off `release/v0.9`, now merged with `release/v0.10.0` (merge 5e78a85)), PR'd back into `release/v0.10.0`. `release/**` runs the i18n RELEASE-TIER gate (`I18N_RELEASE_TIER=1`), so the 3 new keys from Phase 2 must be filled in the 10 base locales before that gate passes (see state.md). The final QA phase offers to delete this `lazy-locales/` directory before the PR settles.
