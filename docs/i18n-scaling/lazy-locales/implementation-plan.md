...
**STEP 0 - PRE-FLIGHT.** Verify `git status` is clean (a concurrent session may share this checkout - if dirty, ask). Confirm you are on `feature/i18n-lazy-locales` (create it off `release/v0.10.0` if it does not exist yet: `git switch -c feature/i18n-lazy-locales`; the branch was originally cut off `release/v0.9`). Scan Claude Code memory for the i18n domain (`MEMORY.md` + `i18n-resolved-baseline-and-assembly`, `i18n-phase3-lazy-locales-plan`, `shared-worktree-commit-care`).
