...
- The barrel composes `en` exactly as today and is the only place `EnTranslations` / `TranslationKey` / `Leaves` are defined, so `Leaves<typeof en, 6>` is still computed over one assembled `en`. (2026-07-17: `TranslationKey` no longer computes `Leaves`; per toolchain-modernization decision D2 it re-exports the build-generated flat union `translation_keys.generated.ts`. `Leaves` stays defined here but is no longer instantiated.)
...
| `tests/i18n_overlay_key_membership.test.ts` (retired 2026-07-17, D2; membership now enforced by tsc, non-vacuity pinned by `tests/i18n_union_teeth.test.ts`) | per-locale **overlays** `src/ui/i18n.locales/<lang>.ts` | Overlays are unchanged by Phase 3; unaffected by either option. |
