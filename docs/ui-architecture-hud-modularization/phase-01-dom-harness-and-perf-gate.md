...
- Agent A (DOM env wiring + fake world + harness test):
  ...
  - Write tests/helpers/fake_world.ts: a hand-rolled IWorld stub that satisfies the IWorld read surface structurally, with simple mutators to drive state for tests: player, target, entities, inventory, equipment, known, marketInfo, socialInfo (plus any other read members the Hud constructor / update path touches per the Explore brief). It must stay behind IWorld and reach into NO Sim or ClientWorld internals.
  - Write tests/hud_harness.test.ts: actually construct the real Hud against fake_world.ts in the DOM env, drive a frame or two, and assert that rendered player-visible text comes from t() (not raw English literals) for at least one representative label. This is the characterization harness the later extraction phases reuse.
  - Deliverables: edited package.json (dep) and vite.config.ts (per-file opt-in only), tests/helpers/fake_world.ts, tests/hud_harness.test.ts.
...
- state.md: update "Current phase"; in the ledger ("Created by this packet"), confirm the three new files landed; record the resolution of the happy-dom canvas OPEN item under OPEN items / gotchas (state explicitly whether the minimap/character-preview canvas was handled by stubbing the canvas 2D context in fake_world.ts or by opting that one file to jsdom, and which files use which env).
...
- If the chosen DOM env cannot render the minimap or character-preview canvas without flipping the global Vitest env, STOP the workaround and resolve it by either stubbing the canvas 2D context in fake_world.ts or opting only that one file to jsdom. Record the resolution in state.md OPEN items. Do NOT flip the global env to make it pass.
