...
Goal: move the {Window} window out of src/ui/hud.ts into src/ui/hud/{window}.ts verbatim, as a class that takes HudContext and exposes render()/toggle()/update(); Hud instantiates it and delegates, with every DOM selector and every signature string byte-identical.
...
- Spawn ONE Explore agent. Tell it to read and summarize, for THIS window only:
  ...
  - The HudContext seam: src/ui/hud/hud_context.ts (what helpers it already exposes), src/ui/hud/hot_write_gate.ts (the one shared gate), src/ui/hud/reactive_diff.ts (ReactiveDiff/StructuralDiff), src/ui/hud/icon_service.ts.
...
- Agent A - Extraction. Re-grep the {Window} symbols in src/ui/hud.ts (anchors drift; trust the symbol, not any line number). Move the toggle{Window}/render{Window}/update{Window} methods (and any private helpers used only by them) VERBATIM into a new class in src/ui/hud/{window}.ts that takes HudContext in its constructor and exposes render()/toggle()/update(). Rewire internal references: this.setText -> ctx.gate.setText (the one shared HotWriteGate, never a new cache), this.closeOtherWindows -> ctx.windows.closeOtherWindows, icon helpers -> ctx.icons, money/number/date formatting -> ctx.formatMoney/formatNumber/formatDateTime (or Intl), world reads -> ctx.sim (IWorld), t -> ctx.t. Keep every #id / .class selector and every signature string byte-identical. Deliverable: src/ui/hud/{window}.ts plus the exact list of HudContext members it consumed.
- Agent B - Delegation. After A lands, make Hud construct the {Window} module once (threading the single shared HudContext, so the gate is shared by reference) and delegate its toggle/render/update calls to the module. Remove the now-dead methods from hud.ts. Deliverable: the hud.ts delegation diff with no orphaned code or imports.
- Agent C - Test. Write tests/hud_{window}.test.ts (opt the file into the DOM env per-file with `// @vitest-environment happy-dom`, or jsdom if the window needs a canvas 2D context, e.g. Character's preview). Assert the window opens/closes/updates against the fake world (tests/helpers/fake_world.ts) and that its rendered text comes from t(). Deliverable: tests/hud_{window}.test.ts green.
...
- No touching the per-frame core, the window manager itself, the matchers (except Social, which only consumes existing matcher output), or any other window.
...
- `npx tsc --noEmit`
- `npm run build`
- With `npm run dev` running, open the {Window} window in the browser and confirm it opens, closes, and updates exactly as before.
...
- [ ] {Window} lives in its own module src/ui/hud/{window}.ts consuming HudContext, exposing render()/toggle()/update().
...
- [ ] The window opens, closes, and updates in `npm run dev`.
