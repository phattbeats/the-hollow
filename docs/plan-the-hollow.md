...
**Sister Shade (the Willow of Wit, SHIPPED as a four-quest arc).** A plain human woman with a watering can, first met marker-free on the Mossbank shore, collecting water with no explanation. Her thesis, Board-locked verbatim: "You don't have to be great to be something good. Greatness isn't kindness." What she never states: she is the Willow of Wit, and the water is a promise to the Plant she has never once missed; the reveal is late and overheard, never in her own line. Her questline is reward-INVERTED (tiny unheroic tasks, meaning as payoff, NO STATS EVER; the one keepsake is the cosmetic `willow_sprig` charm). All four quests are live sim content: quests 1 and 3 shipped under PHAA-558 (PR #146), quests 2 and 4 under PHAA-614 (PR #199). The full writing home, quest-by-quest dialogue, gate history (the PHAA-559 traversal constraints, the lorebook v2.1 finale spine), and deferred beats live in `docs/design/shade-questline.md`; canon sources are the PHAA-544 brief and lorebook v2.1 on PHAA-429.
...
| **2 — The God and the Prophet (the heart)** | The Plant as a rationed, mood-driven LLM-NPC per §5; Greenpaw live with the hunger loop and smoke-as-mood. **In parallel** (Decision 23, §12): the open-world Hollow starter zone (terrain, spawn, roads, reusing the proven terrain system), the vase/Under-Shrine reopened as a portal-reached dungeon, and Homestead v0 (player-chosen, collision-avoided plot placement plus edit-around-yourself, gated behind the full Greenpaw quest arc) on top of it. | Feeding Greenpaw changes the room, the Plant leans in and says something that lands, and Brandon **laughs unprompted.** The starter zone and Homestead v0 do not gate this; they are additional Phase 2 scope, judged on their own when ready. |
...
19. **The slice's hub is portal-instanced** (the base's proven temple pattern). Open-world hub terrain work deferred to Phase 4+ growth. **Partly superseded by Decision 23**: the open-world terrain work is pulled forward to Phase 2.
...
23. **Homestead v0 is a second, later housing tier, and its starter-zone ground is pulled into Phase 2.** Decided on [PHAA-417](/PHAA/issues/PHAA-417) (2026-07-02), Board-approved. Distinct from the Sanctum (Decision-less, shipped on [PHAA-403](/PHAA/issues/PHAA-403)): the Sanctum is a fixed hub-slot placement players get almost immediately after the vase; Homestead v0 is player-chosen placement somewhere viable in the open world (Fallout 76 style, collision-avoided spacing, generous per-plot space), with immediate world-editing around the plot (terrain paint, prop placement, radius; scope still needs a design pass), granted only as the reward for completing the full Greenpaw quest arc. This has no ground to stand on inside the current sealed, no-exit hub, so it reverses part of Decision 19: the vase/Under-Shrine becomes a portal-reached dungeon and a real open-world Hollow starter zone (terrain, spawn, roads, reusing the proven terrain system) grows around it, undoing the no-exit-portal mechanic shipped in Phase 1 (`leaveDungeon` no-op, no exit portal, new characters spawn straight into the hub). Sequenced into Phase 2 (§9), parallel with the God/Prophet work, specifically so it does not add surface to the Phase 1 gate window.
