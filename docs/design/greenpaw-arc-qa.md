...
- **Verified against:** `origin/main` at `1ec710504` (includes the sustained
  lean-in, PR #157 `101c162b3`). Re-verify against code when they disagree; trust
  the code (`docs/CLAUDE.md`).
- **Verdict: PASS.** Every currently-built feature of the arc has direct automated
  coverage and all of it is green (104/104 in the six suites below). One arc beat
  is intentionally NOT built yet; it is called out under "Gaps" so it is tracked,
...
| Four-quest chain: gating, ordering, choice branches, rewards/keepsakes | `src/sim/content/hollow.ts` | `hollow.test.ts`: "q_what_fills carries the full branching offer dialog", "q_the_wavelength unlocks behind q_what_fills and teaches the trainer + hearth loop", "q_the_wavelength ... can be refused for full rewards", "q_keep_him_lit unlocks behind q_the_wavelength and needs three separate feeds", "the quest loot and rewards resolve to real items on the right mobs" | PASS |
...
| Homestead claim reaches the Plant threshold wiring | `housing.ts`, `plant_speech.ts` `notifyThreshold` | `plant_speech.test.ts`: "claiming a homestead reaches the Plant through the real Housing wiring"; `homestead.test.ts` | PASS |

## Gaps and notes for QA to keep tracking

- **Homestead-unlock finale (arc beat 3) is NOT built.** `docs/plan-the-hollow.md`
  Decision 23 gates Homestead plot-claiming behind completing the Greenpaw arc, but
  the claim path has **no quest gate today**: `housingClaim` in `src/sim/housing.ts`
  guards only "stand on a plot" and "you already own one", nothing about quest
  progress. The pitched fix (a `q_your_own_hearth` capstone that sets a flag the
  claim then requires) is unwritten and waiting on a creative-direction nod. Until
  it lands there is nothing to test here; this row is the reminder that the arc is
  not closed.
- **Doc nit for `greenpaw-arc.md` (PR #182):** it names the claim function
  `claimPlot`; the actual symbol on `main` is `housingClaim`. Substance (no quest
  gate) is correct. Worth fixing the name when that doc merges so a future reader
...
| `src/sim/content/hollow.ts` | the 4 quests + dialogue tree data -> `tests/hollow.test.ts` |
...
| `src/sim/housing.ts` | plot claim + Plant threshold -> `tests/homestead.test.ts`, `tests/plant_speech.test.ts` |
