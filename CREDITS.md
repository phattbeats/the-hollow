...
| Per-slot armor meshes (`public/models/armor/helm_plate.glb`, `chest_cape.glb`, `legs_plate.glb`, the first batch of PHAA-502 T2b / PHAA-609 standalone armor GLBs; **derived** from the KayKit Character Pack Adventures 1.0 by extracting the Knight_Helmet, Knight_HelmetVisor, Knight_Cape, Knight_LegLeft, and Knight_LegRight skinned sub-meshes into separate GLBs that share the original Rig_Medium skeleton; pipeline in `scripts/phaa609_extract_armor_glbs.mjs`, smoke-tested in `tests/phaa609_armor_glb_smoke.test.ts`) | Kay Lousberg (KayKit) | https://github.com/KayKit-Game-Assets/KayKit-Character-Pack-Adventures-1.0 | CC0 1.0 |
